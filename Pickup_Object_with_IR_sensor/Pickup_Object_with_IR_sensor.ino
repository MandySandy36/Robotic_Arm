...
// Function to set arm to rest position
...
void setup() {
  ...
  setRestPosition(); // Put the arm in the rest position
}

void loop() {
  // Wait for the IR sensor to detect an object
  if (digitalRead(irSensorPin) == LOW) { // Assume LOW means object detected
    ...
    setRestPosition();
    Serial.println("Object placed. Returning to rest position.");
  } else {
    // If no object detected, stay in the rest position
    Serial.println("Waiting for object...");
    delay(1000); // Delay to avoid continuous polling
  }
}
